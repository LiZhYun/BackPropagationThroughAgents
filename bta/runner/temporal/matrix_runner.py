    ...
    def collect(self, step):
        ...
        if self.use_action_attention:
            ...
            if self.discrete:
                ...
                # mode_item = torch.gather(mixed_, -1, hard_actions.long())
                # mixed_ = torch.clamp(mixed_, max=mode_item)
                # act_bias_ = gumbels.clone()
                # max_values, _ = mixed_.max(dim=-1, keepdim=True)
                # gathered_values = torch.gather(mixed_, -1, hard_actions.long())
                # max_mode_diff = max_values - gathered_values
                # result = gathered_values + max_mode_diff.detach()
                # mixed_.scatter_(dim=-1, index=hard_actions.long(), src=result)
                ind_dist = FixedCategorical(logits=logits)
                mix_dist = FixedCategorical(logits=mixed_)
                # act_mix_dist = FixedCategorical(logits=act_bias_)
            else:

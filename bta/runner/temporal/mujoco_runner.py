...
class MujocoRunner(Runner):
    ...
    def collect(self, step):
        ...
        hard_actions = np.zeros((self.n_rollout_threads, self.num_agents, self.agent_layer, self.action_shape), dtype=np.int32)
        action_log_probs = np.zeros((self.n_rollout_threads, self.num_agents, self.agent_layer, self.action_shape))
        ...
        for idx, agent_idx in enumerate(ordered_vertices):
            ...
            hard_actions[:, agent_idx, idx//self.num_agents] = _t2n(action)
    ...
    def collect_eval(self, step, eval_obs, eval_rnn_states, eval_masks):
        ...
        hard_actions = np.zeros((self.n_eval_rollout_threads, self.num_agents, self.agent_layer, self.action_shape), dtype=np.int32)

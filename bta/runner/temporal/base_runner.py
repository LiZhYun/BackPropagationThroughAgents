        ...
        for epoch in range(self.ppo_epoch):
            ...
            for batch_idx in range(self.num_mini_batch):
                ...
                for idx, agent_idx in enumerate(reversed(ordered_vertices)):
                    ...
                    self.trainer[agent_idx].policy.actor_optimizer.zero_grad()
                    if self.inner_clip_param == 0.:
                        torch.sum(torch.prod(torch.exp(new_actions_logprob-old_action_log_probs_batch),dim=-1, keepdim=True), dim=-1, keepdim=True).mean().backward()
                    else:
                        torch.sum(torch.prod(torch.clamp(torch.exp(new_actions_logprob-old_action_log_probs_batch), 1.0 - self.inner_clip_param, 1.0 + self.inner_clip_param),dim=-1, keepdim=True), dim=-1, keepdim=True).mean().backward()
                    for i in range(self.num_agents):
                        if self.discrete:

...
class HanabiRunner(Runner):
    ...
    def run(self):
        ...
        self.turn_father_actions = np.zeros((self.n_rollout_threads,*self.buffer.father_actions.shape[2:]), dtype=np.float32)       
    ...
    def collect(self, step):
        one_hot_actions = np.zeros((self.n_rollout_threads, self.num_agents, self.all_args.n_actions), dtype=np.float32)
        for current_agent_id in range(self.num_agents):
            ...
            value, action, action_log_prob, rnn_state, rnn_state_critic, father_actions \
                = self.trainer.policy.get_actions(self.use_share_obs[choose],
                                                ...
                                                one_hot_actions=one_hot_actions[choose])
    ...
    def eval(self, total_num_steps):
        ...
        while True:
            if eval_finish:
                break
            eval_one_hot_actions = np.zeros((self.n_eval_rollout_threads, self.num_agents, self.all_args.n_actions), dtype=np.float32)
            for agent_id in range(self.num_agents):
                ...
                eval_action, eval_rnn_state = self.trainer.policy.act(eval_obs[eval_choose],
                                                                ...
                                                                eval_one_hot_actions[eval_choose],
                                                                eval_available_actions[eval_choose],
                                                                deterministic=True,
                                                                agent_id=agent_id,
                                                                one_hot_actions=eval_one_hot_actions[eval_choose])

                eval_actions[eval_choose] = _t2n(eval_action)
                eval_one_hot_actions[eval_choose, agent_id] = _t2n(F.one_hot(eval_action.long(), self.all_args.n_actions).squeeze(1))

        ...
        if self._use_proper_time_limits:
            ...
            if self._use_gae:
                ...
                if self.use_action_attention:
                    ...
                    rho_bar = np.cumprod(truncated_weights, axis=0)
                    for step in reversed(range(self.rewards.shape[0])):
                        rho = rho_bar[step + 1] if step < self.rewards.shape[0] - 1 else 1
                        if self._use_popart or self._use_valuenorm:
                            ...
                            delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
                            gae = delta + rho_bar[step + 1] * self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                            self.returns[step] = gae + self.value_preds[step]
                else:
                    for step in reversed(range(self.rewards.shape[0])):
                        if self._use_popart or self._use_valuenorm:

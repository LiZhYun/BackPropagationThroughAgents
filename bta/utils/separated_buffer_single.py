...
from bta.utils.util import check, get_shape_from_obs_space, get_shape_from_act_space
...
class SeparatedReplayBuffer(object):
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        actions = _cast(self.actions) if len(self.actions.shape) == 3 else self.actions.transpose(1, 0, 2, 3).reshape(-1, *self.actions.shape[2:])
        action_log_probs = _cast(self.action_log_probs) if len(self.action_log_probs.shape) == 3 else self.action_log_probs.transpose(1, 0, 2, 3).reshape(-1, *self.action_log_probs.shape[2:])
        ...
        if self.available_actions is not None:
            available_actions = _cast(self.available_actions[:-1]) if len(self.available_actions.shape) == 3 else self.available_actions[:-1].transpose(1, 0, 2, 3).reshape(-1, *self.available_actions.shape[2:])

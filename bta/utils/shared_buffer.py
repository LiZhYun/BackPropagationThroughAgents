...
class SharedReplayBuffer(object):
    def __init__(self, args, num_agents, obs_space, share_obs_space, act_space, n_rollout_threads=None):
        self.args = args
        ...
        self._predict_shaped_info_horizon = getattr(args, "predict_shaped_info_horizon", None)
        self._predict_shaped_info_event_count = getattr(args, "predict_shaped_info_event_count", None)
    ...
    def compute_shaped_info_target(self):
        assert hasattr(self, "other_shaped_info")
        ret = np.zeros_like(self.other_shaped_info)[:-1]
        for e in range(self.n_rollout_threads):
            for a in range(self.other_shaped_info.shape[2]):
                w = 0
                for t in range(ret.shape[0]):
                    while w + 1 <= self.episode_length and (self.other_shaped_info[t+1: w + 2, e, a]).sum() <= self._predict_shaped_info_event_count:
                        w += 1
                    ret[t, e, a] = self.other_shaped_info[t+1: w + 1, e, a].sum(axis=0)
                    if ret[t, e, a].sum() > 0:
                        ret[t, e, a] = ret[t, e, a] / ret[t, e, a].sum()
        return ret
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self.args.env_name == "GoBigger":
            share_obs = flatten(flatten(self.share_obs[:-1]))
            obs = flatten(flatten(self.obs[:-1]))
        ...
        if hasattr(self, "other_shaped_info"):
            other_shaped_info = self.compute_shaped_info_target().reshape(-1, self.other_shaped_info.shape[-1])
        else:
            other_shaped_info = None
        ...
        for indices in sampler:
            ...
            if other_shaped_info is not None:
                other_shaped_info_batch = other_shaped_info[indices]
            else:
                other_shaped_info_batch = None
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, other_shaped_info_batch, other_policy_id_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        if hasattr(self, "other_shaped_info"):
            other_shaped_info = self.compute_shaped_info_target().reshape(-1, batch_size, self.other_shaped_info.shape[-1])
        else:
            other_shaped_info = None
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                if other_shaped_info is not None:
                    other_shaped_info_batch.append(other_shaped_info[:, ind])
                if other_policy_id is not None:
            ...
            if other_shaped_info is not None:
                other_shaped_info_batch = np.stack(other_shaped_info_batch, 1)
            if other_policy_id is not None:
            ...
            if self.available_actions is not None:
                ...
                available_actions_batch = None
            if other_shaped_info is not None:
                other_shaped_info_batch = _flatten(T, N, other_shaped_info_batch)
            else:
                other_shaped_info_batch = None
            if other_policy_id is not None:
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if hasattr(self, "other_shaped_info"):
            other_shaped_info = _cast(self.compute_shaped_info_target())
        else:
            other_shaped_info = None
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                if other_shaped_info is not None:
                    other_shaped_info_batch.append(other_shaped_info[ind:ind+data_chunk_length])
                if other_policy_id is not None:
            ...
            if other_shaped_info is not None:
                other_shaped_info_batch = np.stack(other_shaped_info_batch, axis=1)
            if other_policy_id is not None:
            ...
            if self.available_actions is not None:
                ...
                available_actions_batch = None
            if other_shaped_info is not None:
                other_shaped_info_batch = _flatten(L, N, other_shaped_info_batch)
            else:
                other_shaped_info_batch = None
            if other_policy_id is not None:

...
from bta.envs.hanabi.Hanabi_Env import HanabiEnv
from bta.envs.env_wrappers import ChooseSubprocVecEnv, ChooseDummyVecEnv
...
def main(args):
    ...
    if all_args.use_wandb:
        ...
        sweep_id = wandb.sweep(sweep_config, project=all_args.env_name + '_' + all_args.experiment_name + '_sweep')
    ...
    # run experiments
    if "gcs" in all_args.algorithm_name:
        from bta.runner.gcs.hanabi_runner import HanabiRunner as Runner
    elif "ar" in all_args.algorithm_name:
        from bta.runner.ar.hanabi_runner import HanabiRunner as Runner
    elif "happo" in all_args.algorithm_name:
        from bta.runner.happo.hanabi_runner import HanabiRunner as Runner
    elif "temporal" in all_args.algorithm_name:
        from bta.runner.temporal.hanabi_runner import HanabiRunner as Runner
    else: # mappo
        from bta.runner.mappo.hanabi_runner import HanabiRunner as Runner

    # sweep
    def train(wconfig=None):
        with wandb.init(config=wconfig,project=all_args.env_name + '_' + all_args.experiment_name + '_sweep',entity=all_args.wandb_name,name=str(all_args.algorithm_name) + "_" +
                            str(all_args.experiment_name) +
                            "_seed" + str(all_args.seed),group=all_args.experiment_name,dir=str(run_dir),):

...
from bta.utils.util import tile_images
...
def gobiggerworker(remote, parent_remote, env_fn_wrapper):
    parent_remote.close()
    env = env_fn_wrapper.x()
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            share_obs, ob, ob_raw, reward, done, info = env.step(data)

            if 'bool' in done.__class__.__name__:
                if done:
                    share_obs, ob, ob_raw = env.reset()
            else:
                if np.all(done):
                    share_obs, ob, ob_raw = env.reset()

            remote.send((share_obs, ob, ob_raw, reward, done, info))
        elif cmd == 'reset':
            share_obs, ob, ob_raw = env.reset()
            remote.send((share_obs, ob, ob_raw))
        elif cmd == 'render':
            if data == "rgb_array":
                fr = env.render(mode=data)
                remote.send(fr)
            ...
            ob, ob_raw = env.reset_task()
            remote.send(ob, ob_raw)
        elif cmd == 'close':
            env.close()
            remote.close()
            break
...
class GoBiggerSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        """
        envs: list of gym environments to run in subprocesses
        """
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=gobiggerworker, args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                   for (work_remote, remote, env_fn) in zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
            p.daemon = True  # if the main process crashes, we should not cause things to hang
            p.start()
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        share_obs, obs, obs_raw, rews, dones, infos = zip(*results)
        return np.stack(share_obs), np.stack(obs), np.stack(obs_raw), np.stack(rews), np.stack(dones), infos

    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        share_obs, obs, obs_raw = zip(*[remote.recv() for remote in self.remotes])
        return np.stack(share_obs), np.stack(obs), np.stack(obs_raw)
...
def shareworker_mujoco(remote, parent_remote, env_fn_wrapper):
    parent_remote.close()
    env = env_fn_wrapper.x()
    while True:
        cmd, data = remote.recv()
...
class ShareSubprocVecEnv_Mujoco(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        """
        envs: list of gym environments to run in subprocesses
        """
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=shareworker_mujoco, args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                   for (work_remote, remote, env_fn) in zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
            p.daemon = True  # if the main process crashes, we should not cause things to hang
            p.start()

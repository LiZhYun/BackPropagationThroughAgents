...
class ACTLayer(nn.Module):
    ...
    def forward(self, x, available_actions=None, deterministic=False, rsample=True, tau=1.0, joint=False):
        if self.mixed_action :
            ...
            if deterministic:
                ...
                if joint:
                    action_log_probs = action_logits.log_probs(actions).sum(-2)
                else:
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None, rsample=False, tau=1.0, kl=False, joint_actions=None, joint=False):
        if self.mixed_action:
            ...
            action_logits = self.action_out(x)
            if joint:
                action_log_probs = action_logits.log_probs(action).sum(-2)
            else:
        ...
        if rsample:
            if self.continuous_action:
                train_actions = action_logits.rsample()
            elif self.discrete_action:
            ...
            if kl:
                action_log_probs_kl = action_logits.log_probs(joint_actions).sum(-2)
            else:

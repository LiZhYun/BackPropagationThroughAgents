...
class R_MAPPO():
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        values, action_log_probs, dist_entropy, _, _ = self.policy.evaluate_actions(share_obs_batch,
                                                                              share_obs_batch, 
                                                                              rnn_states_batch, 
    ...
    def imitate_(self, sample, update_actor=True):
        """
        Update actor and critic networks.
        :param sample: (Tuple) contains data batch with which to update networks.
        :update_actor: (bool) whether to update actor network.
        ...
        policy_loss = (-torch.sum(action_log_probs,
                                             dim=-1,
                                             keepdim=True) * active_masks_batch).sum() / active_masks_batch.sum()
    ...
    def imitate(self, buffer, update_actor=True):
        """
        Perform a training update using minibatch GD.
        :param buffer: (SharedReplayBuffer) buffer containing training data.
        :param update_actor: (bool) whether to update actor network.
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights \
                    = self.imitate_(sample, update_actor)

...
class R_PPOPolicy:
    def __init__(self, args, obs_space, share_obs_space, act_space, device=torch.device("cpu")):
        ...
        #     self.act_space = Categorical(inputs_dim, action_dim, use_orthogonal, gain)
        # elif act_space.__class__.__name__ == "Box":
        #     self.continuous_action = True
        #     action_dim = act_space.shape[0]
        #     self.act_space = DiagGaussian(inputs_dim, action_dim, use_orthogonal, gain)
        self.actor = R_Actor(args, self.share_obs_space, self.act_space, self.device)
